...
import { fetchExchangeRates, convertPrice, type ExchangeRates, type ConvertedPrices } from '@/utils/currency-convert';
...
export default function DynamicPricingPlans() {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-5xl mx-auto">
      {/* Currency selector */}
      <div className="col-span-1 md:col-span-3 flex justify-center gap-2 mb-2">
        {(['DKK', 'USD'] as const).map(currency => (
          <Button
            key={currency}
      ...
      {data.map((plan, index) => {
        ...
        const displayPrice = convertedPrices?.[selectedCurrency] || `${plan.fee.currencySymbol}${plan.fee.amountFormatted}`;
